...
na +420 776131313.';
    ...
    public function MailZavodnikovi($mail){
       ...
       // $mail1->Subject = $this->NazevZavodu. ", predbezna registrace";
        ...
        if($this->IdZavodu == 45){//Cross Country Open
            ...
            $mail1->Body .= '<hr />';
			$mail1->Body .= "<p><b style='color:red'>V případě, pokud se přihlásíte a na závody nakonec nepojedete, dejte prosím vědět na info@timechip.cz, ať se zbytečně netiskne přihláška, díky.</b></p>";
            $mail1->Body .= 'František Krzák<br/>';
            $mail1->Body .= 'E-mail: <a href="f.krzak@seznam.cz">f.krzak@seznam.cz</a><br />';
            $mail1->Body .= '<hr />';
        }



       elseif($this->IdZavodu == 2){
		...
		elseif($this->IdZavodu == 3){ //Perun
			...
			$mail1->Body .= 'Platba – SkyMarathon – 25€<br />';
			$mail1->Body .= 'Variabilni symbol - '.$this->vs.'<br />';
			$mail1->Body .= 'Do zprávy pro příjemce vyplň – PERUN SKYMARATHON<br />';
			...
			$mail1->Body .= 'Payment – SkyMarathon – 25€<br />';
			...
			$mail1->Body .= 'Those who have paid the entry payment till March 30, 2022 will be entitled to get a t-shirt.  Unfortunatelly it´s not posiible after this date because the concrete number of registered and paid T-shirts are going  to print !!! So  don´t forget about it. And again like last year Perun helps mothers with disabled children in association called NIKDY NEJSI SÁM  (you are never alone) so some part of your payment goes to their account. <br />';
			$mail1->Body .= 'looking forward on the start line of Perun 2020<br />';
			...
			elseif($this->IdZavodu == 5 || $this->IdZavodu == 13 || $this->IdZavodu == 46){ //hOBBY
            ...
            $mail1->Body .= "Telefon: {$this->udaje['telefon_1']}<br />";
            $mail1->Body .= '<hr />';
            /* zakomentovano kvuli gunrace, nechtelo se mi delat zvlast podminku
	...
	private function StartovneZDB(){
	    $sql1 = "SELECT * FROM prihlasky_startovne_".YEAR." WHERE prihlasky_startovne_".YEAR.".id_zavodu = $this->IdZavodu AND poradi_podzavodu = {$this->udaje['poradi_podzavodu']}";
	   // echo $sql1;
	    $sth1 = $this->db->prepare($sql1);
	    $sth1->execute();
	    if($sth1->rowCount()){
	...
	private function Startovne(){
            if ($this->IdZavodu == 2) { //winter sikland race
                if($this->udaje['id_kategorie'] < 5 OR $this->udaje['id_kategorie'] > 8){
				...
				if($this->udaje['id_kategorie'] == 9) $this->startovne_z_db_kc = 2300;
					$this->vychozi_startovne = $this->startovne_z_db_kc * $pocet_clenu;
                	$this->startovne_kc = $this->vychozi_startovne;
            }
			elseif($this->IdZavodu == 11){
	...
	public function VypisPrihlasek(){
	    //$this->Prevod();
	    $str = false;
	    if($this->IdZavodu ==45){ // cc licence
		$str .= $this->VypisPrihlasekEnduro();
	    }
	    elseif($this->IdZavodu == 46){ //cc hobby
	...
	private function TymyEnduro(){
	    $sql1 = "SELECT id_tymu,nazev_tymu FROM tymy WHERE id_enduro > 0 ORDER BY nazev_tymu";
	    $sth1 = $this->db->prepare($sql1);
	    $sth1->execute();
	    if($sth1->rowCount()){
		$sth1->setFetchMode(PDO::FETCH_OBJ);
