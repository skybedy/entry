...
<div class="row">
	...
	/*
        $str .= '<div class="form-group">';
        $str .= '<select name="tricko_'.$i.'" class="form-control placeholder  required"><option value="" selected>Tričko</option><option value="bez">Bez trička</option>';

        foreach($tricka as $key => $val){
            $str .= '<option value="'.$val.'"'.((isset($udaje['tricko_'.$i]) && $udaje['tricko_'.$i] == $val) ? 'selected="selected"' : '').'>'.$val.'</option>';
        }
        $str .= '</select>';
        $str .= '</div>'; */
        $str .=  '</div>';
