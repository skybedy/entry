...
<div class="row">
	...
	$str .= '<div class="col-lg-4">';
	$str .= '<div class="form-group">';
	$str .= '<select name="ponozky_'.$i.'" class="form-control required placeholder">';
	$str .= '<option value="">Číslo ponožek</option>';
	foreach($this->ponozky as $key => $val){
	    $str .= '<option value="'.$val.'"'.((isset($udaje['ponozky_'.$i]) && $udaje['ponozky_'.$i] == $val) ? 'selected="selected"' : '').'>'.$key.'</option>';
	}
	$str .= '</select>';
	$str .= '</div></div>';
	$str .= '</div>';
	...
	$str .= '<option value="">Tričko (+5€, nepovinné)</option>';
	...
	$str .= '<div class="form-group">';
	$str .= '<input name="dalsi_udaje_1_'.$i.'" class="form-control" placeholder="Značka kola (třeba pro případ odcizení, nepovinné)" value="'.(isset($udaje['dalsi_udaje_1_'.$i]) ? $udaje['dalsi_udaje_1_'.$i] : '').'"/>';
	$str .= '</div>';
	$str .= '<div class="checkbox">';
	$str .= '<label>';
	$str .= '<input type="checkbox" name="dalsi_udaje_2_'.$i.'" value="DATATAG" /> Bezpečnostný system <a target="_blank" href="http://datatag.co.uk/website-slovak">DATATAG</a> na bicykel SK alebo CZ  (30€, nepovinné)';
	$str .= '</label>';
	$str .= '</div>';
	$str .= '<div class="checkbox">';
	$str .= '<label>';
	$str .= '<input type="checkbox" name="dalsi_udaje_3_'.$i.'" value="Uterak" />  Uterák (ručník) - <a style="text-decoration: underline" target="_blank" href="http://www.bikepoint.sk/photos/obrazok_158.jpg">náhled</a> (17€, nepovinné)';
	$str .= '</label>';
	$str .= '</div>';
	$str .= '<br />';
