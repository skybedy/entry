$(function(){
   ...
   //Backbone.emulateJSON = true;
    ...
    Handlebars.registerHelper('Readonly', function(racer_detail){
        var str = '';
	if(racer_detail != null){
	    if(racer_detail.length > 0){
		return ' readonly';
	    }
	}
    });
    
    Handlebars.registerHelper('Disabled', function(racer_detail){
        var str = '';
	if(racer_detail != null){
	    if(racer_detail.length > 0){
		return ' readonly';
    ...
    EnduroForm = Backbone.View.extend({
     ...
     // alert();
	    ...
	    var verification_data = $(ev.currentTarget).serializeObject();
            ...
            that = this;
            $.getJSON('./prihlaska/overeni-udaju-enduro',verification_data,function(xhr){
                if(xhr.ido.length > 0){
                    that.render(xhr);
                }
                else{
                    var template = '<div class="panel panel-default hlaska"><div class="panel-body">';
		    template = template += 'Kombinace startovního čísla a e-mailové adresy je neplatná, <a href="./prihlaska">zkuste to znovu</a>.<br />';
		    template = template += 'V případě, že nebude úspěšní opakovaně, kontaktujte nás prostřednictvím e-mailu na <a href="mailto://info@timechip.cz">info@timechip.cz</a>, nebo telefonicky na číslo +420 776 131313.';
        ...
        render: function(racer_data){
            ...
            category_list_server.fetch({
               ...
               success:function(category_list_server){
                ...
                data:{ido:racer_data.ido,startovni_cislo:racer_data.startovni_cislo},
    ...
    EnduroControlTable = Backbone.View.extend({
        ...
        events: {
            "submit #prihlasovaci_formular_enduro": "racerDetailsFormular",
            "click #navrat_do_formulare": "racerDetailsStorage"
        },
        racerDetailsFormular: function(ev){
            ev.preventDefault();
    ...
    var OdeslaniPrihlasky = Backbone.View.extend({
        el: "#prihlasovaci_formular_enduro_wrapper",
        template : Handlebars.compile( $("#hlaska_po_prihlaseni").html()),
        events: {
            "click #odeslat_prihlasku": "hlaska_po_prihlaseni"
        },

	hlaska_po_prihlaseni: function(ev){
            ev.preventDefault();
            var racer_data = JSON.parse(sessionStorage.racer_details);
	    this.render(racer_data);
	},

	render: function(racer_data){
	    //preloader, určitě by šel do kodu implementovat elegantněji, ale už nemám čas
	    ...
	    that = this;
	    $.getJSON('./prihlaska/ulozeni-prihlasky-enduro',racer_data,function(xhr){
		    if(xhr.uspesne_prihlaseni != false){
		    sessionStorage.removeItem('racer_details');
		}
		that.$el.html(that.template({hlaska:xhr.hlaska}));
	    });
	}
